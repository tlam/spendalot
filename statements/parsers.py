...
class MastercardParser(object):
    ...
    def parse(self, row):
        ...
        transaction_datetime = datetime.strptime(
            transaction_datetime, "%m/%d/%Y %I:%M %p"
        )
        amount = Decimal(amount)
        if amount <= Decimal(0):
            return None
        ...
        if Expense.objects.filter(
            description=description, date=transaction_datetime, amount=amount
        ).count():
            return None
...
class ParserFactory(object):
    PARSERS = [HistoricalParser, LegacyMastercardParser, MastercardParser]
